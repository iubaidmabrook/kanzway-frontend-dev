...
async function MyOrder() {
  ...
  const TAB_ITEMS = [
    {
      key: 'all',
      value: t('profile.allOrder'),
    },
    {
      key: 'on_placed',
      value: t('profile.onPlaced'),
    },
    {
      key: 'on_process',
      value: t('profile.onProcess'),
    },
    {
      key: 'on_delivery',
  ...
  const orderList = () => {
    ...
    let selectedList: TOrderList[] = [];

    switch (activeFilter) {
      case 'all':
        selectedList = FAKE_ORDER;
        break;
      case 'on_placed':
        selectedList = FAKE_ORDER.filter((o) => o.status === 'On placed');
        break;
      case 'on_process':
        selectedList = FAKE_ORDER.filter((o) => o.status === 'On Process');
        break;
      case 'on_delivery':
        selectedList = FAKE_ORDER.filter((o) => o.status === 'On Delivery');
        break;
      case 'completed':
        selectedList = FAKE_ORDER.filter((o) => o.status === 'Completed');
        break;
      case 'cancel':
        selectedList = FAKE_ORDER.filter((o) => o.status === 'Canceled');
        break;
          ...
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="col-sm-auto col-7 ">
              <div className="d-flex align-items-center justify-content-end gap-2">
                <div className="text-xxs color-dark text-bold">
                  {`${t('common.sar')} ${items.grandTotal}`}
                </div>
                <Link
                  href={`profile/myorder/${items.id}`}
