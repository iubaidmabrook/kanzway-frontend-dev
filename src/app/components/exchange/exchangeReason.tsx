...
function ExchangeReason({ exchange }: { exchange: TExchangeDetail }) {
  const t = useI18n();
  ...
  return (
    <div className="box-border bg-white rounded-2 p-3 mb-3">
      ...
      >
        {t('profile.exchangeReason')}
      </span>
      <div
        className={`${isCollapse ? '' : 'show'} collapse`}
        id="link-collapse-2"
      >
        <div className="text-xs my-2 color-dark">{exchange.reason}</div>
        <Swiper
          loop={false}
          slidesPerView="auto"
          spaceBetween={8}
        ...
        >
          {exchange.images.map((imagesItems) => (
