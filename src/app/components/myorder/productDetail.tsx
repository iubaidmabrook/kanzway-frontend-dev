...
function ProductDetail({ order }: { order: TOrderDetail }) {
  const t = useI18n();
  ...
  const renderProduct = () => {
    ...
    const orderProduct: Array<TOrderItem>[] = [];
    for (let i = 0; i < orderItems.length; i++) {
      orderProduct.push(orderItems[i].products);
    }

    let newOrderProducts: Array<TOrderItem> = [];
    for (let i = 0; i < orderProduct.length; i++) {
      newOrderProducts = newOrderProducts.concat(orderProduct[i]);
    }

    return newOrderProducts.map((items, index, row) => (
      <>
